...
class Board:
    ...
    def calc_legal_moves(self):
        ...
        for pos in self.board.keys():
            #Check white
            if self.board[pos] == WHITE_SYMBOL:
                for direction in DIRECTIONS.keys():
                    if self.check_dir_with_jumps(pos, direction):
                        white_legal_moves += 1
            
            #Check black
            elif self.board[pos] == BLACK_SYMBOL:
                for direction in DIRECTIONS.keys():
                    if self.check_dir_with_jumps(pos, direction):
                        black_legal_moves += 1
...
class Strategy:
    ...
    def bfs(self, board, start, symbol):
        """ 
        ...
        while queue:
            ...
            for direction in DIRECTIONS.keys():
                ...
                if new_pos not in board.board:
                    continue
                
                #Add the new position to the queue if it is not in the queue 
                #and it hasnt been visited and if it is not blocked by 
                #another piece or place on the board
                if new_pos not in queue and new_pos not in visited and\
                        not self.is_blocked(board, new_pos, symbol):
                    parents[new_pos] = pos 
